...
object gcounter {
  final case class IntGCounter(counters: Map[String, Int]) {
    def increment(machine: String, amount: Int): IntGCounter =
      IntGCounter(counters + (machine -> (amount + counters.getOrElse(machine, 0))))
  ...
  final case class MapGCounter[K, V](counters: Map[K, V]) {
    def increment(machine: K, amount: V)(implicit M: Monoid[V]): MapGCounter[K, V] =
      MapGCounter(counters + (machine -> (amount |+| counters.getOrElse(machine, M.empty))))

    def merge(that: MapGCounter[K, V])(implicit B: BoundedSemilattice[V]): MapGCounter[K, V] =
  ...
  trait GCounter[F[_, _], K, V] {
    ...
    def merge(f2: F[K, V], f1: F[K, V])(implicit B: BoundedSemilattice[V]): F[K, V]
  ...
  implicit class KvsOps[F[_,_], K, V](f: F[K, V]) {
    def put(key: K, value: V)
      (implicit kvs: KeyValueStore[F]): F[K, V] = kvs.put(f)(key, value)

    def get(key: K)(implicit kvs: KeyValueStore[F]): Option[V] = kvs.get(f)(key)

    def getOrElse(key: K, default: V)
      (implicit kvs: KeyValueStore[F]): V = kvs.getOrElse(f)(key, default)
  ...
  implicit def gcounterInstance[F[_,_], K, V]
    (implicit KVS: KeyValueStore[F], M: Monoid[F[K, V]]): GCounter[F, K, V] =
      new GCounter[F, K, V] {
        def increment(f: F[K, V])(key: K, value: V)(implicit MV: Monoid[V]): F[K, V] =
